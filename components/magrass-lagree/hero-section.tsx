...
export function MagrassHeroSection() {
  ...
  return (
    <section className="relative overflow-hidden bg-white py-16 sm:py-20 lg:py-24">
      <div
        className="absolute inset-0 bg-[url('/magrass-lagree/hero-poster.webp')] bg-cover bg-center opacity-[0.06]"
        aria-hidden
      />
      ...
      <div className="mx-auto max-w-7xl px-6 md:px-12">
        ...
        <div className="mt-8 grid gap-4 sm:mt-10 sm:grid-cols-3 sm:gap-6">
          {valuePillars.map((pillar) => (
            <article
              key={pillar.title}
              className="rounded-2xl border border-mag-border bg-white p-5 shadow-sm sm:rounded-3xl sm:p-6"
            >
