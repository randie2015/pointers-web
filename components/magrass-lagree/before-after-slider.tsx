...
export function MagrassBeforeAfterSlider() {
  ...
  return (
    <div className="relative mx-auto mt-8 w-full max-w-5xl sm:mt-10">
      <div
        ref={containerRef}
        className="relative touch-none overflow-hidden rounded-2xl border-2 border-mag-gold/40 bg-mag-navy shadow-xl select-none sm:rounded-3xl"
      ...
      >
        ...
        <div className="pointer-events-none absolute inset-y-0 z-10 w-0.5 -translate-x-1/2 bg-white shadow-lg" style={{ left: `${position}%` }}>
          <div className="absolute top-1/2 left-1/2 flex h-11 w-11 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border-[3px] border-white bg-mag-gold shadow-lg">
        ...
        <span className="pointer-events-none absolute top-3 right-3 z-20 rounded-full bg-mag-gold/95 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-mag-navy sm:top-4 sm:right-4 sm:px-3 sm:text-xs">
          Después
        </span>
      </div>
