...
const themeStyles = {
  light: {
    name: 'text-demo-charcoal',
    tagline: 'text-demo-accent-deep'
  },
  dark: {
    name: 'text-white',
    tagline: 'text-demo-soft'
  }
} as const;

export function ClinicLogo({ demo, className, size = 'md', theme = 'light' }: ClinicLogoProps) {
  ...
  return (
    <span className={cn('flex min-w-0 items-center gap-2.5 sm:gap-3', className)}>
      ...
      <span className="min-w-0 shrink font-cinzel leading-none">
        <span className={cn('block font-semibold uppercase transition-colors duration-500', palette.name, nameStyles[size])}>
          {demo.brand.name}
        </span>
